...
//        String mediaMid = mapper1.getMapper("file").getString("media_mid");
    ...
    public  List<Music> albumInfoToAlbumMusic(JSONObject jsonObject, QQConfig qqConfig) {
        ...
        jsonArray.forEach(jdata->{
            ...
            Integer indexAlbum = e.getJSONObject("songInfo").getInteger("index_album");
            Music music = new Music().setId(mid).setMusicName(string).setMusicAlbum(albumname).setMusicArtists(singerNames).setArtistsIds(singerids).setMusicImage(url).setDataInfo(e).setBits(longs).setTrackNumber(indexAlbum);
            collect.add(music);
        });
